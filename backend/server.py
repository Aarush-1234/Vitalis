...
app = FastAPI(title="Vitalis Mission Monitoring API", version="2.0.0")
...
async def analyze_health_risk(astronaut_id: str, current_data: Dict) -> Dict[str, Any]:
    ...
    if hr_deviation > 2:
        risk_factors.append({
            ...
            "message": "Heart rate significantly deviates from personal baseline",
...
async def get_alerts(astronaut_id: str, alert_status: str = "active"):
...
async def health_check():
    return {
        ...
        "version": "2.0.0",
        "system": "Vitalis Mission Monitoring"
    }


# ===================== BIOMETRIC SCAN ENDPOINTS =====================
...
async def store_biometric_scan(data: BiometricScanInput):
    """Store biometric data captured from webcam facial analysis"""
    record = {
        "id": str(uuid.uuid4()),
        "astronaut_id": data.astronaut_id,
        "heart_rate": data.heart_rate,
    ...
    return {
        ...
        "record_id": record["id"],
        "risk_analysis": risk_analysis,
        "message": "Biometric scan data stored successfully"
    }


@app.get("/api/biometric/latest/{astronaut_id}")
async def get_latest_biometric_scan(astronaut_id: str):
    """Get the most recent biometric scan for an astronaut"""
    record = await health_data_col.find_one(
        {"astronaut_id": astronaut_id, "source": "webcam_biometric"},
        sort=[("timestamp", -1)]
    )
    if record:
        record.pop("_id", None)
